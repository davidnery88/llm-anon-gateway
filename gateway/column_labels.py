...
class ColumnLabelStore:
    ...
    async def upsert(
        ...
        # Upsert sans contexte (db, table) = entrée générique. Le caller qui
        # voudrait upserter une entrée contextualisée doit utiliser upsert_ctx
        # (à venir) — pas critique pour l'usage actuel.
        row = await self._pool.fetchrow(
            """
            ...
            RETURNING id, header_norm, header_raw, label, source, status,
                      confidence, occurrences, sample_values, db_name, table_name,
    ...
    async def list_all(self, status: str | None = None) -> list[dict]:
        if status is None:
            rows = await self._pool.fetch(
                ...
                "confidence, occurrences, sample_values, db_name, table_name, created_at, updated_at "
...
async def seed_static_map(pool) -> int:
    ...
    for header_norm, label in STATIC_HEADER_MAP.items():
        # Phase 4.5 : la contrainte unique est désormais composite
        # (header_norm, db_name, table_name). Pour le seed générique, db et
        # table sont NULL → on cible la combinaison NULLS NOT DISTINCT.
        result = await pool.execute(
            """
            INSERT INTO column_labels
                (header_norm, header_raw, label, source, status, confidence, db_name, table_name)
            VALUES ($1, $2, $3, 'static', 'active', 1.0, NULL, NULL)
            ON CONFLICT (header_norm, db_name, table_name) DO NOTHING
            """,
